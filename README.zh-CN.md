...
完成首次扫描后，说出 **"复扫"** 或 **"re-audit"** 即可对同一领域进行跟进扫描。Agent 会：

1. **在 `docs/blind-spot-scan/` 查找上次报告**
2. **向你确认**："是对 `blind-spot-scan-[domain]-YYYY-MM-DD.md` 这份报告进行 re-audit 吗？"
3. **复用相同维度**，但轮换题目——保留 1–2 道测试记忆，其余换成更深入的新题
4. **生成 Diff 附录**，对比上次与本次的覆盖率变化：
...
**Top improvement:** Data Pipeline — 从近空白到可跟教程
**Regression alert:** Risk Management — 尾部风险概念退步
```

Diff 使用纯文本格式（无 markdown 表格），在任何渲染器下都不会崩。趋势箭头：+10% 及以上 → 📈 | −10% 及以下 → 📉 | 其他 → ➡️。
...
复制以下指令发送给你的 AI Agent，Agent 会自动从 GitHub 克隆并安装：

```
请从 https://github.com/Randallyi/blind-spot-scanner.git 克隆 blind-spot-scanner skill，
放到你的 skills 目录下（例如 <agent-root>/skills/blind-spot-scanner/）。
```
...
4. **可重复审计 + Diff 对比**  
   2–4 周后的复扫不是重置，而是求差值。Diff 附录精确显示哪些维度进步、哪些退步、以及原因，把"我感觉学会了"变成"Data Pipeline +27%，但 Risk Management 退了 −13%"。复扫由用户主动触发，Agent 不会自动运行。
